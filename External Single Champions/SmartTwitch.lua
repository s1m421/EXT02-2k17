    ...
    for i = 1, Game.HeroCount() do
      ...
      if hero.isEnemy and isValidTarget(hero,E.Range) then
        local stack = EStacks(hero)
			local TargetERange = STarget(E.Range)
			...
			if SmartTwitchMenu.Misc.AutoE:Value() or SmartTwitchMenu.Key.ComboKey:Value() and SmartTwitchMenu.Combo.UseE:Value() then
                    ...
                    else if SmartTwitchMenu.Key.HarassKey:Value() and SmartTwitchMenu.Harass.UseE:Value() and myHero.mana/myHero.maxMana > SmartTwitchMenu.Harass.ManaE:Value()/100 and SmartPoison[target.networkID].stacks >= SmartTwitchMenu.Harass.HarassEStackAmmount:Value() then 
                    Control.CastSpell(HK_E)
        						--Print("Harass E on X Stacks")
								--end --Old end of elseif
      				end
							else if SmartTwitchMenu.Misc.AutoERange:Value() and SmartTwitchMenu.Misc.AutoERangeStacks:Value() and SmartPoison[target.networkID].stacks >= SmartTwitchMenu.Misc.AutoERangeStacks:Value() and TargetERange ~= nil and TargetERange and ETargetPos and GetDistance(myHero.pos, TargetERange.pos) > 1025 then
							Control.CastSpell(HK_E)
										--Print("Auto E On MAX RANGE")
							end
								else if SmartTwitchMenu.Misc.EBeforeDeath:Value() and myHero.health/myHero.maxHealth < SmartTwitchMenu.Misc.HPToEBeforeDeath:Value()/100 and CountEnemy(myHero.pos,550) > 0 then
